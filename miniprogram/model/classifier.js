...
var Classifier = /** @class */ (function () {
    ...
    Classifier.prototype.classify = function (ab, size) {
        ...
        var start = Date.now();
        tf.tidy(function () {
            var pixels = tf.tensor3d(data, [size.height, size.width, 4], 'float32')
                .slice([0, 0, 0], [-1, -1, 3]);
            var tensor = _this.mobileNet.predict(pixels);
            var topK = _this.mobileNet.getTopKClasses(tensor, 5);
            result += "prediction: " + (Date.now() - start) + "ms\n";
            result += topK.map(function (x, i) { return x.value.toFixed(3) + ": " + x.label; }).join('\n');
            return result;
        });
        this.page.setData({ result: result });
    };
